...
async def start_cmd(message: Message, state: FSMContext):
    ...
    if unsubbed:
        ...
        btn = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"Kanal {i+1}", url=url, style="primary")] for i, url in enumerate(unsubbed)
...
async def get_receipt(message: Message, state: FSMContext):
    ...
    # Admin uchun: Tasdiqlash — success (yashil) | Bekor qilish — danger (qizil)
    btn = InlineKeyboardMarkup(inline_keyboard=[[
...
async def send_support(message: Message, state: FSMContext):
    ...
    btn = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="Javob yozish", callback_data=f"reply_{message.from_user.id}", style="primary")
    ]])
...
async def approve_ad(call: CallbackQuery):
    ...
    if ad:
        ...
        # Sotuvchi bilan bog'lanish — success (yashil)
        # Reklama berish — primary (ko'k)
        btn = InlineKeyboardMarkup(inline_keyboard=[
            [
...
async def del_ch_step(call: CallbackQuery):
    ...
    if not channels:
        ...
        return
    # O'chirish tugmalari — danger (qizil)
    btn = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"O'chirish: {ch[1]}", callback_data=f"delch_{ch[0]}", style="danger")] for ch in channels
    ])
