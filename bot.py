...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # ===== UC SOTIB OLISH JADVALLARI =====
        c.execute('''CREATE TABLE IF NOT EXISTS uc_prices 
                     (id INTEGER PRIMARY KEY AUTOINCREMENT, uc_amount TEXT, price TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS uc_orders 
                     (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, full_name TEXT, username TEXT,
                      uc_amount TEXT, price TEXT, pubg_id TEXT, receipt_file_id TEXT,
                      status TEXT DEFAULT 'pending', created_at TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS uc_card 
                     (id INTEGER PRIMARY KEY, card_number TEXT)''')
        c.execute("INSERT OR IGNORE INTO uc_card (id, card_number) VALUES (1, '8600 0000 0000 0000')")
        # =====================================
...
class UCOrderForm(StatesGroup):
    pubg_id_photo = State()  # PUBG ID screenshoti
...
def get_uc_card():
    res = db_query("SELECT card_number FROM uc_card WHERE id=1", fetchone=True)
    return res[0] if res else "Karta kiritilmagan"
...
async def menu_ad_cb(message: Message, state: FSMContext):
    ...
    if posted >= (1 + paid):
        ...
        btn = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="💳 To'lov qilish", callback_data="pay_ad")
        ]])
        await message.answer(
...
# ==========================================
# ===== PUBG MOBILE UC OLISH MODULI =====
# ==========================================

@router.message(F.text == "🎮 PUBG MOBILE UC OLISH")
async def uc_menu(message: Message, state: FSMContext):
    await state.clear()
    prices = db_query("SELECT id, uc_amount, price FROM uc_prices ORDER BY id", fetchall=True)
    ...
    # UC narxlari tugmalari — 2 tadan
    buttons = []
    row = []
    for i, (pid, uc_amount, price) in enumerate(prices):
        row.append(InlineKeyboardButton(
            text=f"💎 {uc_amount} UC — {price} so'm",
            callback_data=f"uc_buy_{pid}"
        ))
        if len(row) == 2:
            buttons.append(row)
            row = []
...
@router.callback_query(F.data.startswith("uc_buy_"))
async def uc_buy_cb(call: CallbackQuery, state: FSMContext):
    pid = int(call.data.split("_")[2])
    row = db_query("SELECT uc_amount, price FROM uc_prices WHERE id=?", (pid,), fetchone=True)
    if not row:
        await call.answer("Bu narx topilmadi!", show_alert=True)
        return
    ...
    await call.message.edit_text(
        f"💎 <b>{uc_amount} UC</b> — <b>{price} so'm</b>\n\n"
        f"📌 Buyurtma berish uchun:\n"
        f"1️⃣ <b>PUBG ID</b> raqamingiz ko'rsatilgan screenshotni yuboring\n\n"
        f"📸 <i>Screenshot qanday olish:</i> PUBG Mobile → Profil → ID raqam ko'rinib tursin",
        parse_mode="HTML"
    )
    await state.set_state(UCOrderForm.pubg_id_photo)

@router.message(UCOrderForm.pubg_id_photo, F.photo)
async def uc_get_pubg_id_photo(message: Message, state: FSMContext):
    photo_id = message.photo[-1].file_id
    await state.update_data(pubg_photo=photo_id)
    ...
    await message.answer(
        f"✅ PUBG ID screenshoti qabul qilindi!\n\n"
        f"💳 Endi to'lov qiling:\n\n"
        f"<b>Karta raqami:</b> <code>{card}</code>\n"
        f"<b>Summa:</b> <code>{price} so'm</code>\n\n"
        f"💰 To'lov qilgach, <b>chek (screenshot)</b>ni yuboring:",
        parse_mode="HTML"
    )
    await state.set_state(UCOrderForm.receipt)

@router.message(UCOrderForm.pubg_id_photo)
async def uc_pubg_photo_wrong(message: Message):
    await message.answer("❌ Iltimos, PUBG ID ko'rsatilgan <b>rasm (screenshot)</b> yuboring!", parse_mode="HTML")

@router.message(UCOrderForm.receipt, F.photo)
async def uc_get_receipt(message: Message, state: FSMContext):
    data = await state.get_data()
    receipt_id = message.photo[-1].file_id
    uc_amount = data['uc_amount']
    price = data['uc_price']
    pubg_photo = data.get('pubg_photo', '')
    now = get_time_tashkent()

    # DB ga saqlash
    order_id = db_query(
        "INSERT INTO uc_orders (user_id, full_name, username, uc_amount, price, pubg_id, receipt_file_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (message.from_user.id, message.from_user.full_name,
         message.from_user.username or "", uc_amount, price,
         pubg_photo, receipt_id, now)
    )

    # Admin uchun tugmalar
    admin_btn = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Tasdiqlash", callback_data=f"uc_approve_{order_id}"),
        InlineKeyboardButton(text="❌ Rad etish", callback_data=f"uc_reject_{order_id}")
    ]])
    ...
    caption = (
        ...
        f"🕐 Yuborilgan vaqt: <b>{now}</b> (Toshkent vaqti)"
    )

    # Avval PUBG ID screenshotini yubor
    await bot.send_photo(
        ADMIN_ID,
        photo=pubg_photo,
        caption=f"📸 <b>PUBG ID Screenshot</b>\n👤 {message.from_user.full_name} (ID: {message.from_user.id})",
        parse_mode="HTML"
    )

    # Keyin chekni yuborish
    await bot.send_photo(
        ADMIN_ID,
        photo=receipt_id,
        caption=caption,
    ...
    await message.answer(
        ...
        "📋 Adminlar ko'rib chiqadi va UC tez orada hisobingizga tushiriladi.\n"
        "⏱ Odatda 5-30 daqiqa ichida bajariladi.",
        reply_markup=get_main_menu(),
        parse_mode="HTML"
    )
    await state.clear()
...
async def uc_receipt_wrong(message: Message):
    await message.answer("❌ Iltimos, to'lov chekining <b>rasm (screenshot)</b>ini yuboring!", parse_mode="HTML")

# ===== UC ADMIN CALLBACK =====
@router.callback_query(F.data.startswith("uc_approve_"))
async def uc_approve(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        await call.answer("Sizda ruxsat yo'q!", show_alert=True)
        return
    order_id = int(call.data.split("_")[2])
    order = db_query("SELECT user_id, uc_amount, price FROM uc_orders WHERE id=?", (order_id,), fetchone=True)
    if not order:
        await call.answer("Buyurtma topilmadi!", show_alert=True)
        return
    user_id, uc_amount, price = order
    db_query("UPDATE uc_orders SET status='approved' WHERE id=?", (order_id,))
    try:
        await bot.send_message(
            user_id,
            f"✅ <b>UC buyurtmangiz tasdiqlandi!</b>\n\n"
            f"💎 <b>{uc_amount} UC</b> tez orada PUBG hisobingizga tushiriladi.\n"
            f"Rahmat!",
            parse_mode="HTML",
            reply_markup=get_main_menu()
        )
...
async def uc_reject(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        await call.answer("Sizda ruxsat yo'q!", show_alert=True)
        return
    order_id = int(call.data.split("_")[2])
    order = db_query("SELECT user_id, uc_amount FROM uc_orders WHERE id=?", (order_id,), fetchone=True)
    if not order:
        await call.answer("Buyurtma topilmadi!", show_alert=True)
        return
    ...
    try:
        await bot.send_message(
            user_id,
            f"❌ <b>UC buyurtmangiz rad etildi.</b>\n\n"
            f"💎 {uc_amount} UC buyurtmasi bekor qilindi.\n"
            f"Muammo bo'lsa adminga murojaat qiling: 🆘 Yordam",
            parse_mode="HTML",
            reply_markup=get_main_menu()
        )
    except:
...
# ==========================================
# ===== ADMIN PANELGA UC NARX QO'SHISH =====
# ==========================================

@router.message(Command("admin"), F.from_user.id == ADMIN_ID)
async def admin_panel(message: Message):
    btn = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Statistika (Rasmli)", callback_data="admin_stats")],
        [
        ...
        # ===== UC NARX TUGMALARI =====
        [InlineKeyboardButton(text="💎 UC narxi kiritish", callback_data="admin_uc_add")],
        [InlineKeyboardButton(text="📋 UC narxlarini ko'rish", callback_data="admin_uc_list")],
        [InlineKeyboardButton(text="💳 UC karta raqami", callback_data="admin_uc_card")],
    ])
    await message.answer("⚙️ Admin panelga xush kelibsiz!", reply_markup=btn)

# UC narx qo'shish
@router.callback_query(F.data == "admin_uc_add")
async def uc_add_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer("💎 UC miqdorini kiriting (masalan: 60 UC yoki 300 UC):")
    await state.set_state(AdminForm.uc_amount)

@router.message(AdminForm.uc_amount)
async def uc_add_step2(message: Message, state: FSMContext):
    await state.update_data(uc_amount=message.text)
    await message.answer(f"💰 {message.text} uchun narxni kiriting (so'mda, faqat raqam):")
    await state.set_state(AdminForm.uc_price)

@router.message(AdminForm.uc_price)
async def uc_add_save(message: Message, state: FSMContext):
    data = await state.get_data()
    db_query("INSERT INTO uc_prices (uc_amount, price) VALUES (?, ?)", (data['uc_amount'], message.text))
    await message.answer(f"✅ <b>{data['uc_amount']}</b> — <b>{message.text} so'm</b> narxi qo'shildi!", parse_mode="HTML")
    await state.clear()

# UC narxlarini ko'rish va o'chirish
@router.callback_query(F.data == "admin_uc_list")
async def uc_list_cb(call: CallbackQuery):
    prices = db_query("SELECT id, uc_amount, price FROM uc_prices ORDER BY id", fetchall=True)
    if not prices:
        await call.message.answer("📭 Hozircha UC narxlari yo'q.")
        return
    rows = []
    for pid, uc_amount, price in prices:
        rows.append([InlineKeyboardButton(
            text=f"🗑 O'chirish: {uc_amount} — {price} so'm",
            callback_data=f"uc_del_{pid}"
        )])
    markup = InlineKeyboardMarkup(inline_keyboard=rows)
    text = "\n".join([f"💎 {ua} — {pr} so'm" for _, ua, pr in prices])
    await call.message.answer(f"📋 <b>UC narxlari:</b>\n\n{text}", reply_markup=markup, parse_mode="HTML")

@router.callback_query(F.data.startswith("uc_del_"))
async def uc_del_cb(call: CallbackQuery):
    pid = int(call.data.split("_")[2])
    db_query("DELETE FROM uc_prices WHERE id=?", (pid,))
    await call.message.edit_text("✅ Narx o'chirildi.")

# UC karta raqamini o'zgartirish
@router.callback_query(F.data == "admin_uc_card")
async def uc_card_step(call: CallbackQuery, state: FSMContext):
    current = get_uc_card()
    await call.message.answer(f"💳 Hozirgi karta: <code>{current}</code>\n\nYangi karta raqamini kiriting:", parse_mode="HTML")
    await state.set_state(AdminForm.uc_card)

@router.message(AdminForm.uc_card)
async def uc_card_save(message: Message, state: FSMContext):
    db_query("UPDATE uc_card SET card_number=? WHERE id=1", (message.text,))
    await message.answer(f"✅ UC karta raqami yangilandi:\n<code>{message.text}</code>", parse_mode="HTML")
    await state.clear()

# ==========================================
# ===== QOLGAN MAVJUD HANDLERLAR =====
# ==========================================
...
async def send_support(message: Message, state: FSMContext):
    btn = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="↩️ Javob yozish", callback_data=f"reply_{message.from_user.id}")
    ]])
        ...
        try:
            if site_url:
                ...
                markup = InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(text="📝 shu yerni bosib kodni kiriting", web_app=WebAppInfo(url=unlock_url))
                ]])
                await bot.send_message(
                    send_to,
                    f"✅ <b>To'lovingiz tasdiqlandi!</b>\n\n"
                    f"Quyidagi tugmani bosing — sayt ochiladi va e'lon formasi avtomatik ochiladi:\n\n"
                    f"🔑 Kod: <code>{code}</code>\n(Tugma ishlamasa kodni qo'lda kiriting)",
                    parse_mode="HTML", reply_markup=markup
                )
            else:
                await bot.send_message(
                    send_to,
                    f"✅ <b>To'lovingiz tasdiqlandi!</b>\n\n🔑 <code>{code}</code>\n\n⚠️ Kod faqat <b>1 marta</b> ishlaydi!",
                    parse_mode="HTML"
                )
            sent = True
        except Exception:
