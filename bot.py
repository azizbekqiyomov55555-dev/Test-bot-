...
async def approve_web_pay(call: CallbackQuery):
    ...
    # Captiondan username ajratib olish
    username = None
    ...
    # Maxsus token yaratish (sayt URL ichiga joylashadi)
    import random, string, hashlib, time
    raw = f"{username}-{time.time()}-{random.randint(1000,9999)}"
    token = hashlib.md5(raw.encode()).hexdigest()[:16]

    # Tokenni DB ga saqlash
    db_query("CREATE TABLE IF NOT EXISTS unlock_tokens (token TEXT PRIMARY KEY, used INTEGER DEFAULT 0, created INTEGER)")
    db_query("INSERT OR REPLACE INTO unlock_tokens (token, used, created) VALUES (?, 0, ?)", (token, int(time.time())))

    # Sayt URL ni olish
    site_url = get_setting('site_url') or "https://YOUR_SITE.com"

    # Foydalanuvchiga havolali xabar yuborish
    unlock_url = f"{site_url}?token={token}"
    sent = False
    if username:
        try:
            await bot.send_message(
                ...
                f"Quyidagi tugmani bosing va yangi e'lon joylang:\n\n"
                f"🔗 {unlock_url}",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(
                        text="📝 Yangi E'lon Joylash",
                        url=unlock_url,
                        style="success"
                    )
                ]])
    ...
    # Admin uchun ham ko'rsatish
    note = f"\n\n✅ TO'LOV TASDIQLANDI"
    note += f"\n🔗 {unlock_url}"
    note += f"\n📨 @{username} ga yuborildi" if sent else f"\n⚠️ Avtoyuborib bo'lmadi — havolani qo'lda yuboring:\n{unlock_url}"

    await call.message.edit_caption(caption=caption + note, reply_markup=None)
    await call.answer("✅ Tasdiqlandi!" + (" Havola yuborildi." if sent else " Havolani qo'lda yuboring!"), show_alert=True)
...
# ================== TOKEN TEKSHIRISH (DB orqali, server kerak emas) ==================
# Token URL da keladi: https://site.com?token=abc123
# Sayt tokenni localStorage ga saqlaydi va keyingi yuklanishda tekshiradi
# Bu yerda bot admin panelida sayt URL ni sozlash uchun:
