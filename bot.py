...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        # TEZLIK: WAL rejimi — bir paytda o'qish va yozish parallel
...
def db_query(query, params=(), fetchone=False, fetchall=False):
    # TEZLIK: timeout va check_same_thread=False — bloklanmaslik uchun
...
async def check_subscription(user_id):
    ...
    if not channels:
        return []
    # TEZLIK: barcha kanallarni PARALLEL tekshirish (ketma-ket emas)
    async def _check(ch_id, url):
        try:
            ...
            return None
        return None
    results = await asyncio.gather(*[_check(ch_id, url) for ch_id, url in channels])
...
async def main():
    ...
    # TEZLIK: eski yangilanishlarni o'tkazib yuborish + faqat kerakli update turlari
