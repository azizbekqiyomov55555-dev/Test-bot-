...
# ================== ASOSIY MENU — INLINE RANGLI ==================
def get_main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            # E'lon berish — SUCCESS (yashil)
            InlineKeyboardButton(text="📝 E'lon berish", callback_data="menu_ad", style="success"),
            # Yordam — DANGER (qizil)
            InlineKeyboardButton(text="🆘 Yordam", callback_data="menu_help", style="danger"),
        ]
    ])
...
async def start_cmd(message: Message, state: FSMContext):
    await state.clear()
    # Eski kulrang ReplyKeyboard o'chirish
...
async def menu_ad_cb(call: CallbackQuery, state: FSMContext):
    await call.answer()
    unsubbed = await check_subscription(call.from_user.id)
    if unsubbed:
        await call.message.answer("Iltimos, oldin kanallarga obuna bo'ling. /start ni bosing.")
        return

    user = db_query("SELECT posted_ads, paid_slots, pending_approval FROM users WHERE user_id=?", (call.from_user.id,), fetchone=True)
    ...
    await call.message.answer("E'loningizni boshlaymiz.\nIltimos, akkaunt obzori videosini yuboring:")
    await state.set_state(AdForm.video)

@router.callback_query(F.data == "menu_help")
async def menu_help_cb(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer("Adminga xabaringizni yozib qoldiring:")
