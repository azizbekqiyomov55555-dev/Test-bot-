...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS users 
                     (user_id INTEGER PRIMARY KEY, full_name TEXT, username TEXT, join_date TEXT, posted_ads INTEGER DEFAULT 0, paid_slots INTEGER DEFAULT 0, pending_approval INTEGER DEFAULT 0)''')
...
def get_main_menu():
    kb = ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text="🟢 📝 E'lon berish"),   # Yashil — E'lon berish
            KeyboardButton(text="🔴 🆘 Yordam")           # Qizil — Yordam
        ]], 
...
async def start_cmd(message: Message, state: FSMContext):
    ...
    if unsubbed:
        btn = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"🔔 Kanal {i+1} — Obuna bo'lish", url=url)] for i, url in enumerate(unsubbed)
...
async def pay_ad_cb(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        f"🟠 💳 To'lov uchun karta raqam:\n\n`{card}`\nSumma: {price} so'm\n\nTo'lov qilgach, chekni rasm (skrinshot) qilib yuboring.",
        parse_mode="Markdown")
...
async def approve_ad(call: CallbackQuery):
    ...
    if ad:
        ...
        # ===== KANALGA JOYLANADIGAN TUGMALAR — RANGLI =====
        btn = InlineKeyboardMarkup(inline_keyboard=[
            [
